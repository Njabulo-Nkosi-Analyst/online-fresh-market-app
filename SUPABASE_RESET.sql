-- =====================================================================
-- Roots & Earth — CLEAN RESET + FRESH SETUP
-- =====================================================================
-- ⚠️  THIS SCRIPT WILL **DELETE** ALL EXISTING ROOTS & EARTH DATA.
--
-- Use this ONCE when you want a clean slate.
-- After running, your Supabase project will have:
--   • profiles, categories, products, orders, order_items, favorites
--   • Row-Level Security policies (incl. guest checkout)
--   • Auto-create-profile trigger on signup
--   • 6 categories + 16 seed products
--
-- HOW TO RUN
...
-- Drop trigger + function first (they reference profiles)
drop trigger if exists on_auth_user_created on auth.users;
drop function if exists public.handle_new_user() cascade;

-- Drop tables (CASCADE removes policies, indexes, fk references)
...
-- Profiles: a user can read / create / update only their own row
...
-- Admins can write categories & products
...
--   • GUESTS (user_id is null) can also create orders → enables guest checkout
...
-- Order items: insertable when the parent order belongs to the user (or is guest),
-- selectable by order owner or admin
...
-- =====================================================================
-- STEP 5 · AUTO-CREATE profile row on signup
-- =====================================================================
...
-- =====================================================================
-- STEP 7 · SEED data — 16 products
-- =====================================================================
insert into public.products
  (name, description, price, compare_at_price, category_id, image_url,
   stock, is_organic, is_best_seller, is_new_arrival, is_limited_deal,
   rating, rating_count, unit) values
  ('Heirloom Tomatoes',       'Heirloom tomatoes sun-ripened on organic vines. Deep flavour, minimal food-miles.', 49.90, 64.90,  (select id from public.categories where slug='vegetables'),     'https://images.unsplash.com/photo-1592924357228-91a4daadcfea?w=600&q=80', 42,  true,  true,  false, true,  4.8, 231, '500g'),
  ...
  ('Grass-fed Butter',        'Cultured butter from grass-fed cows. Deep yellow, rich flavour.',                   79.00, null,   (select id from public.categories where slug='dairy'),          'https://images.unsplash.com/photo-1589985270826-4b7bb135bc9d?w=600&q=80', 18,  true,  false, false, false, 4.8, 74,  '250g'),
...
-- DONE!
-- Next steps:
--   1. Sign up in your live app (any email)
--   2. Supabase → Table Editor → profiles → set is_admin = true on your row
--   3. Refresh your app — admin icon appears in the navbar
-- =====================================================================
