...
const AuthModal = () => {
    ...
    try {
      ...
      if (error) {
        ...
        if (msg.includes('not enabled') || msg.includes('unsupported provider') || msg.includes('validation_failed')) {
          toast.error("Google sign-in isn't available right now. Please use email and password.", { duration: 7000 });
        } else {
    ...
    try {
      ...
      if (error) {
        ...
        if (msg.includes('rate limit') || msg.includes('too many')) {
          toast.error('Too many requests. Please wait a few minutes and check your spam folder.', { duration: 8000 });
        } else {
          toast.error(error.message);
        }
      } else {
  ...
  if (view === 'check-email') {
    return (
      <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
        <div className="relative w-full max-w-md card-surface rounded-2xl p-8 text-center">
          <button onClick={close} className="absolute top-4 right-4 p-2 rounded-full hover:bg-[#262924] text-[#a8a69c]">
            <X className="w-4 h-4" />
          </button>
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      <div className="relative w-full max-w-md card-surface rounded-2xl p-8">
        <button onClick={close} className="absolute top-4 right-4 p-2 rounded-full hover:bg-[#262924] text-[#a8a69c]">
          <X className="w-4 h-4" />
        </button>
        ...
        <p className="text-center text-sm text-[#a8a69c] mt-5">
          ...
          <button
            ...
            className="text-[#d69e4b] hover:text-[#f2f0e6]"
          >
